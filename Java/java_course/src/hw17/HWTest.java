...
public class HWTest {
	...
	static class PanelSouth extends JPanel {
		...
		PanelSouth() {
			...
			label = new JLabel(
					"      Натиснете бутона, чийто цвят е и вашето число");
	...
	static class PanelButtons extends JPanel {
		...
		PanelButtons() {
			this.setLayout(new BorderLayout());
			label = new JLabel(
					"     Натиснете бутона, който съдържа вашето число");
			this.add(label, BorderLayout.NORTH);
			this.setBackground(Color.WHITE);
			label.setFont(newTextFont);

			buttonPan1 = new Button("      5     10     27     33      ",
					Color.RED);
			buttonPan2 = new Button("      18     3     6     8      ",
					Color.YELLOW);

			buttonPan3 = new Button("      2     15     11     19      ",
					Color.CYAN);

			buttonPan4 = new Button("      35     0     22     1      ",
					Color.ORANGE);
